...
1.1 Why Aya 23?
Aya 23 is an open-source, multilingual language model developed by Cohere for Good, explicitly designed to democratise access to AI across linguistic boundaries.
...
Cost: ~R6000-8000/month for GPU instance
...
Note: INT8 quantisation allows deployment on 8GB GPUs (RTX 4060), reducing cost by ~30% with minimal performance impact.
Alternative: Repurposed Gaming PCs
Partner with local gaming cafes or e-sports organisations to repurpose older gaming PCs during school hours. This can reduce initial capital costs significantly.
2.3 Software Stack
Core Components:

Operating System: Ubuntu 24.04 LTS (long-term support, free)
Model Runtime: vLLM or llama.cpp (optimized inference engines)
Web Interface: Custom React frontend with FastAPI backend
Authentication: OAuth 2.0 with school-based user management
Monitoring: Prometheus + Grafana (usage analytics, performance)
Content Filtering: Custom moderation layer for age-appropriate responses
...
Apply for initial seed funding (R300k-400k)
...
Approved textbooks (digitised with publisher permission)
...
Base Model: Start with Aya 23 (8B) checkpoint from HuggingFace
Approach: Parameter-efficient fine-tuning (LoRA) to reduce compute costs
Training Data: ~50,000-100,000 instruction-response pairs per language
Evaluation: Human evaluation by educators + automated benchmarks
Iteration: Monthly model updates based on classroom feedback
...
This initiative can scale to reach hundreds of thousands of South African students within 3-5 years.
